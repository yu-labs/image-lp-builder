...
import { AlertTriangle, ArrowRight, Check, Copy, ExternalLink, X } from 'lucide-react';
...
type CtaButtonMode = NonNullable<Cta['buttonMode']>;
type CtaTextPresetSelection = (typeof CTA_PRESETS)[number]['id'];
...
const CTA_MODAL_INPUT_CLASS = `${EDITOR_INPUT_CLASS} w-full`;
const CTA_MODAL_INPUT_ERROR_CLASS =
  'border-[#e19a9a] bg-[#fffafa] focus:border-[#d98282] focus:ring-[#f7d9d9]';
const CTA_MODAL_ERROR_CLASS =
...
function serializeCtas(ctas: Cta[]): string {
...
function ctaTelError(value: string): string | null {
  const trimmed = value.trim();
...
function ctaEmailError(value: string): string | null {
  const trimmed = value.trim();
...
function validateCtaLinkForEditor(link: CtaLink): string[] {
  switch (link.type) {
    case 'line_friend': {
...
function collectCtaSaveErrors(ctas: Cta[]): SaveErrorGroup[] {
...
function getCtaButtonMode(cta: Cta): CtaButtonMode {
  return cta.buttonMode ?? (cta.image ? 'image' : 'text');
}

function getCtaTextPresetSelection(cta: Cta): CtaTextPresetSelection {
...
function migrateCtaLink(current: CtaLink, next: CtaLink): CtaLink {
  switch (next.type) {
    case 'custom_url': {
      if (
